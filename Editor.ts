...
class Editor extends EventEmitter {
  ...
  private _checkPlugin(plugin: IPluginTempl) {
    ...
    if (this.pluginMap[pluginName]) {
      throw new Error(`Plugin ${pluginName} has already been initialized`);
    }
    events.forEach((eventName: string) => {
      if (this.customEvents.find((info) => info === eventName)) {
        throw new Error(`Duplicate event '${eventName}' in plugin '${pluginName}'`);
      }
    });

    apis.forEach((apiName: string) => {
      if (this.customApis.find((info) => info === apiName)) {
        throw new Error(`Duplicate API '${apiName}' in plugin '${pluginName}'`);
      }
    });
    return true;
  }
